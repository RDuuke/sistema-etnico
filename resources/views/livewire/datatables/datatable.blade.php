                ...
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('app.search_in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                ...
                <div class="items-center justify-between p-2 sm:flex">
                    ...
                    @if(count($this->results))
                        ...
                        <div class="flex justify-end text-gray-600">
                            {{__('app.results')}} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }} {{__('of')}}
                            {{ $this->results->total() }}
                        </div>
                    @endif
                </div>
