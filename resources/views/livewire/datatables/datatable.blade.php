<div>
    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
                ...
                @if($this->searchableColumns()->count())
                    <div class="flex w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-black leading-4 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('app.search_in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if($this->activeFilters)
                    <button wire:click="clearAllFilters" class="flex items-center px-3 text-xs font-medium tracking-wider text-red-500 uppercase bg-white border border-red-400 space-x-2 leading-4 hover:bg-red-200 focus:outline-none"><span>{{ __('Reset') }}</span>
                ...
                @if(count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        ...
                        <select wire:model="massActionOption" class="px-3 text-xs font-medium tracking-wider uppercase bg-white border border-green-400 space-x-2 leading-4 focus:outline-none" id="datatables_mass_actions">
                        ...
                        <button
                            ...
                            class="flex items-center px-4 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 leading-4 hover:bg-green-200 focus:outline-none" type="submit" title="Submit"
                        >Go</button>
                    </div>
                @endif
                ...
                @if($exportable)
                    <div x-data="{ init() {
                        ...
                        <button wire:click="export" class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 space-x-2 leading-4 hover:bg-green-200 focus:outline-none"><span>{{ __('Export') }}</span>
                ...
                @foreach ($columnGroups as $name => $group)
                    <button wire:click="toggleGroup('{{ $name }}')"
                            class="px-3 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 leading-4 hover:bg-green-200 focus:outline-none">
        ...
        @if($hideable === 'buttons')
            <div class="p-2 grid grid-cols-8 gap-2">
                @foreach($this->columns as $index => $column)
                    @if ($column['hideable'])
                        <button wire:click.prefetch="toggle('{{ $index }}')" class="px-3 py-2 text-white text-xs focus:outline-none
        ...
        <div wire:loading.class="opacity-50" class="@unless($complex || $this->hidePagination) @endunless shadow-lg bg-project-primary max-w-screen overflow-x-scroll @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex)-b-none border-b-0 @endif">
            <div>
                <div class="table min-w-full align-middle">
                    @unless($this->hideHeader)
                        <div class="table-row divide-x divide-black">
                            @foreach($this->columns as $index => $column)
                                @if($hideable === 'inline')
                                    @include('datatables::header-inline-hide', ['column' => $column, 'sort' => $sort])
                                @elseif($column['type'] === 'checkbox')
                                    @unless($column['hidden'])
                                        <div class="flex justify-center table-cell w-32 h-12 px-6 py-4 overflow-hidden text-xs font-medium tracking-wider text-left text-gray-500 uppercase align-top border-b border-black bg-gray-50 leading-4 focus:outline-none">
                        ...
                        <div class="table-row p-1 {{ $this->rowClasses($row, $loop) }}">
                            @foreach($this->columns as $column)
                                ...
                                @else
                                    ...
                                    <div class="table-cell border-black border-b px-6 py-2 @unless($column['wrappable']) whitespace-nowrap truncate @endunless @if($column['contentAlign'] === 'right') text-right @elseif($column['contentAlign'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}">
                                        {!! $row->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
            ...
            @if($this->results->isEmpty())
                <p class="p-3 text-lg text-center">
                    {{ __("app.there's_nothing_to_show_at_the_moment") }}
                </p>
            @endif
        </div>

        @unless($this->hidePagination)
            <div class="max-w-screen bg-white @unless($complex)-b-lg @endunless border-4 border-t-0 border-b-0 @if($this->activeFilters) border-blue-500 @else border-transparent @endif">
                <div class="items-center justify-between p-2 sm:flex">
                    {{-- check if there is any data --}}
                    @if(count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage" class="block w-full py-2 pl-3 pr-10 mt-1 text-base border-black form-select leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5" wire:model="perPage">
                        ...
                        <div class="flex justify-end text-gray-600">
                            {{__('app.results')}} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }} {{__('de')}}
                            {{ $this->results->total() }}
                        </div>
                    @endif
                </div>
    ...
    @if($complex)
        <div class="bg-gray-50 px-4 py-4-b-lg-t-none shadow-lg @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) border-t-0 @endif">
